...
docker compose exec backend python -m unittest discover -s tests -v     # backend (login) tests, seconds
```
...
- **`backend/`** (FastAPI, its own deps in `backend/requirements.txt`): imports `wordgame.py` and serves `boards/daily`. Future days return 404. The board endpoint never sends word lists: swipes are checked server-side (`POST /boards/{date}/check` takes cell indexes on the *unrotated* board). Progress is saved in SQLite, keyed by `deps.current_player`: `user:<id>` with a session cookie, else `X-Player-Id` (an anonymous id from the browser). Optional login lives in `auth.py` (routes), `accounts.py` (users, hashed sessions and email tokens), `google.py` (OIDC + PKCE) and `mailer.py` (SMTP); `/api/auth/claim` moves anonymous progress into the account. Every write must be `Content-Type: application/json` (CSRF guard in `main.py`). `/define/{word}` scrapes Milog (it has no CORS).
- **`frontend/`** (Vite + React + TS): base components in `src/components/` (Button, Pill, Chip, Modal, ProgressBar, Tile), screens in `src/features/`, game state in `src/state/useGame.ts`, progress in `src/state/progressStore.ts` (localStorage first, synced to the API), login in `src/state/auth.ts` (`bootAuth` finishes a login and claims before the first sync; logout wipes the device) and `features/AccountModal.tsx`. `lib/layout.ts` maps rotated cells back to the stored ones (`layout.base`).
