...
- `test_wordgame.py` generates real boards, so the full run takes minutes. Only run
  ...
  `build_web.py`, `schedule.json`, `shapes.json`, `themes/`, `data/` or the test file
  itself. **Don't** run it for `frontend/`, `backend/`, `web/template.html` or docs;
  use `cd frontend && npx tsc -b` for the frontend instead. When only one area moved,
  run that class (`TestThemes`, `TestShapes`, `TestSchedule`, `TestGame`, …) and save
  the full suite for the end.
